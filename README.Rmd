...
Below is an overview of how HotellingEllipse can help draw a confidence ellipse:
  ...
  * with `ellipseParam()` we get the T$^2$ statistic along with the values of the semi-minor and semi-major axes of the Hotelling ellipse. Whereas, `ellipseCoord()` provides the *x* and *y* coordinate points for drawing the Hotelling ellipse at user-defined confidence interval. 
...
Another way to add a confidence ellipse is to use the function `ellipseCoord()`. This function provides the *x* and *y* coordinate points of the confidence ellipse at user-defined confidence interval. The confidence interval `confi.limit` is set at 95% by default.
